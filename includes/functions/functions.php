...
/************************************
**  getitems() function  v2.0
**  function to get items from database [users,items,comments]

*/   


function getitems($select,$table,$where,$value)
{
    global $con;
    $getstmt=$con->prepare('SELECT '. $select. ' FROM '.$table.' WHERE '.$where.' = ?' );
 ...
 ** function to check user status in database [with parameters]
 ** $user = the user to check his status if he active or not
 */

function checkUserStatus($user)
{                          
   global $con;    
   $statment= $con->prepare("SELECT Username,RegStatus FROM users WHERE Username = ? AND RegStatus =0");
